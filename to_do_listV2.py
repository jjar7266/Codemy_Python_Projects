...
def add_todo(tasks):
    ...
    tasks.append({"task": item, "completed": False})  # Adds task to list
...
def remove_todo(tasks):
    view_todos(tasks)  # Show tasks first so user knows the numbers

    if tasks:  # Only allow removal if list is not empty
        try:
...
def mark_completed(tasks):
    view_todos(tasks)  # Show tasks so the user knows the numbers

    if tasks:  # Only allow marking if list is not empty
